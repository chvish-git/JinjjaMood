...
import { ArrowLeft, Send, Sparkles, Clock, Calendar } from 'lucide-react';
...
export const MoodCheck: React.FC<MoodCheckProps> = ({ isDark, onBack, onSubmit, username }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Auto-fill timestamp happens in saveMoodLog function
      // TC-MOOD-001: Log mood with journal (TC-MOOD-004: Journal is optional)
      // TC-MOOD-005: Logs are tied to correct userId
      await saveMoodLog({
        mood: selectedMood,
        journalEntry: journalEntry.trim(),
        timestamp: new Date() // This will be auto-filled by the storage function
      }, userProfile.uid);
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-6 py-20">
        ...
        <div className={`text-center mb-8 transform transition-all duration-1000 ${
          ...
          <h1 className={`text-4xl md:text-6xl font-bold mb-4 ${
            ...
            {hasLoggedToday ? 'You\'re all set for today!' : 'How are you feeling right now?'}
          </h1>
